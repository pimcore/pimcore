...
class Dao extends Model\Element\Dao
{
    ...
    public function updateChildPaths($oldPath)
    {
        ...
        $documents = $this->db->fetchCol('SELECT id FROM documents WHERE path LIKE ?', Db::get()->escapeLike($oldPath . '%'));
        ...
        $this->db->query('update documents set path = replace(path,' . $this->db->quote($oldPath . '/') . ',' . $this->db->quote($this->model->getRealFullPath() . '/') . "), userModification = '" . $userId . "' where path like " . $this->db->quote(Db::get()->escapeLike($oldPath . '/%')) . ';');

        //update documents child permission paths
        $this->db->query('update users_workspaces_document set cpath = replace(cpath,' . $this->db->quote($oldPath . '/') . ',' . $this->db->quote($this->model->getRealFullPath() . '/') . ') where cpath like ' . $this->db->quote(Db::get()->escapeLike($oldPath . '/%')) . ';');

        //update documents child properties paths
        $this->db->query('update properties set cpath = replace(cpath,' . $this->db->quote($oldPath . '/') . ',' . $this->db->quote($this->model->getRealFullPath() . '/') . ') where cpath like ' . $this->db->quote(Db::get()->escapeLike($oldPath . '/%')) . ';');
    ...
    public function isLocked()
    {
        ...
        $belowLocks = $this->db->fetchOne("SELECT tree_locks.id FROM tree_locks
            INNER JOIN documents ON tree_locks.id = documents.id
                WHERE documents.path LIKE ? AND tree_locks.type = 'document' AND tree_locks.locked IS NOT NULL AND tree_locks.locked != '' LIMIT 1", Db::get()->escapeLike($this->model->getRealFullPath() . '/%'));
    ...
    {
        $lockIds = $this->db->fetchCol('SELECT id from documents WHERE path LIKE ' . $this->db->quote(Db::get()->escapeLike($this->model->getRealFullPath() . '/%')) . ' OR id = ' . $this->model->getId());
    ...
    public function isAllowed($type, $user)
    {
        ...
        try {
            ...
            if (empty($permissionsParent) && $type == 'list') {
                ...
                $permissionsChildren = $this->db->fetchOne('SELECT list FROM users_workspaces_document WHERE cpath LIKE ? AND userId IN (' . implode(',', $userIds) . ') AND list = 1 LIMIT 1', Db::get()->escapeLike($path . '%'));
