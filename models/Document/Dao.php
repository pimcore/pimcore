...
class Dao extends Model\Element\Dao
{
    ...
    public function updateChildPaths($oldPath)
    {
        ...
        $documents = $this->db->fetchOne('SELECT id FROM documents WHERE path LIKE ?', Db\Helper::escapeLike($oldPath) . '%');
    ...
    {
        $lockIds = $this->db->fetchOne('SELECT id from documents WHERE path LIKE ' . $this->db->quote(Helper::escapeLike($this->model->getRealFullPath()) . '/%') . ' OR id = ' . $this->model->getId());
