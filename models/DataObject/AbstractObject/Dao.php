...
class Dao extends Model\Element\Dao
{
    ...
    public function updateChildPaths($oldPath)
    {
        if ($this->hasChildren(DataObject::$types)) {
            ...
            $objects = $this->db->fetchOne('SELECT o_id FROM objects WHERE o_path LIKE ?', Db\Helper::escapeLike($oldPath) . '%');
            ...
            $this->db->executeQuery('update objects set o_path = replace(o_path,' . $this->db->quote($oldPath . '/') . ',' . $this->db->quote($this->model->getRealFullPath() . '/') . "), o_userModification = '" . $userId . "' where o_path like " . $this->db->quote(Db\Helper::escapeLike($oldPath) . '/%') . ';');

            //update object child permission paths
            $this->db->executeQuery('update users_workspaces_object set cpath = replace(cpath,' . $this->db->quote($oldPath . '/') . ',' . $this->db->quote($this->model->getRealFullPath() . '/') . ') where cpath like ' . $this->db->quote(Db\Helper::escapeLike($oldPath) . '/%') . ';');

            //update object child properties paths
            $this->db->executeQuery('update properties set cpath = replace(cpath,' . $this->db->quote($oldPath . '/') . ',' . $this->db->quote($this->model->getRealFullPath() . '/') . ') where cpath like ' . $this->db->quote(Db\Helper::escapeLike($oldPath) . '/%') . ';');
    ...
    public function isLocked()
    {
        ...
        $belowLocks = $this->db->fetchOne("SELECT tree_locks.id FROM tree_locks INNER JOIN objects ON tree_locks.id = objects.o_id WHERE objects.o_path LIKE ? AND tree_locks.type = 'object' AND tree_locks.locked IS NOT NULL AND tree_locks.locked != '' LIMIT 1", Db\Helper::escapeLike($this->model->getRealFullPath()) . '/%');
    ...
    {
        $lockIds = $this->db->fetchOne('SELECT o_id from objects WHERE o_path LIKE ' . $this->db->quote(Db\Helper::escapeLike($this->model->getRealFullPath()) . '/%') . ' OR o_id = ' . $this->model->getId());
    ...
    public function getClasses()
    {
        if ($this->getChildAmount()) {
            ...
            $classIds = $this->db->fetchOne("SELECT o_classId FROM objects WHERE o_path LIKE ? AND o_type = 'object' GROUP BY o_classId", Db\Helper::escapeLike($path) . '/%');
    ...
    public function isAllowed($type, $user)
    {
        ...
        try {
            ...
            if (empty($permissionsParent) && $type === 'list') {
                ...
                $permissionsChildren = $this->db->fetchOne('SELECT list FROM users_workspaces_object WHERE cpath LIKE ? AND userId IN (' . implode(',', $userIds) . ') AND list = 1 LIMIT 1', Db\Helper::escapeLike($path) . '%');
    ...
    public function getChildPermissions($type, $user, $quote = true)
    {
        ...
        try {
            ...
            $cid = $this->model->getId();
            $sql = 'SELECT ' . $type . ' FROM users_workspaces_object WHERE cid != ' . $cid . ' AND cpath LIKE ' . $this->db->quote(Db\Helper::escapeLike($this->model->getRealFullPath()) . '%') . ' AND userId IN (' . implode(',', $userIds) . ') ORDER BY LENGTH(cpath) DESC';
