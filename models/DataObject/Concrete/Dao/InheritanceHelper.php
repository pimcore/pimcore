        ...
        if ($createMissingChildrenRows) {
            ...
            if ($this->childFound || (empty($this->fields) && empty($this->relations))) {
                ...
                $query = 'SELECT b.o_id AS id '
                    ...
                    . ' AND o_path LIKE '. $this->db->quote(Db::get()->escapeLike($object->getRealFullPath().'/%'))
    ...
    protected function buildTree($currentParentId, $fields = '', $parentIdGroups = null, $params = [])
    {
        ...
        if (!$parentIdGroups) {
            ...
            if (isset($params['language'])) {
                $query = "SELECT a.language as language, b.o_id AS id $fields, b.o_classId AS classId, b.o_parentId AS parentId FROM objects b LEFT JOIN " . $this->storetable . ' a ON b.o_id = a.' . $this->idField . ' WHERE o_path LIKE ' . $this->db->quote(Db::get()->escapeLike($object->getRealFullPath() . '/%'))
                    . ' HAVING `language` = "' . $params['language'] . '" OR ISNULL(`language`)'
                    . ' ORDER BY LENGTH(o_path) ASC';
            } else {
                $query = "SELECT b.o_id AS id $fields, b.o_classId AS classId, b.o_parentId AS parentId FROM objects b LEFT JOIN " . $this->storetable . ' a ON b.o_id = a.' . $this->idField . ' WHERE o_path LIKE ' . $this->db->quote(Db::get()->escapeLike($object->getRealFullPath().'/%')) . ' GROUP BY b.o_id ORDER BY LENGTH(o_path) ASC';
            }
