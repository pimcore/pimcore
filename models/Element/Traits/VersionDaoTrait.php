...
trait VersionDaoTrait
{
    ...
    {
        $versionId = $this->db->fetchOne("SELECT id FROM versions WHERE cid = :cid AND ctype = :ctype AND `date` > :mdate AND ((draft = 1 AND userId = :userId) OR draft = 0) ORDER BY `versionCount` DESC LIMIT 1", [
