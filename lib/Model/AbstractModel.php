...
abstract class AbstractModel implements ModelInterface
{
    ...
    public function initDao($key = null, $forceDetection = false)
    {
        ...
        $dao = self::getDoaClassFor($myClass, $key, $forceDetection);
        ...
        $this->injectDao(new $dao());
    }

    /**
     * @internal
     */
    public static function getDoaClassFor(string $className, ?string $key = null, bool $forceDetection = false): string
    {
        $cacheKey = $className . ($key ? ('-' . $key) : '');
        $dao = null;
        ...
        if (!$forceDetection && array_key_exists($cacheKey, self::$daoClassCache)) {
            ...
            if (isset(self::$daoClassMap[$className])) {
                $dao = self::$daoClassMap[$className];
            } else {
                $dao = self::locateDaoClass($className);
            }
        } else {
    ...
    {
        $blockedVars = ['dao', 'o_dirtyFields', 'activeDispatchingEvents', 'eventDispatcher'];
