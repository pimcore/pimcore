...
class AssetUpdateTasksHandler
{
     ...
     * generating thumbnails) and saves the results. Both are skipped if the processing token of the asset changed
     * since it was loaded (see Asset::getProcessingToken()): its data was replaced or restored in the meantime then,
     * so the results belong to previous data and are discarded, as they would overwrite the state of the current data,
     * whose own task would find its processing finished and skip it (or which doesn't need any processing, as it was
     * restored from a processed state). The lock makes sure that a replacement can't slip in between the check and
     * the save, and that thumbnails generated by the completion can't survive a replacement or restore in the
     * meantime, as the thumbnails are cleared after the asset was saved with the new data, which the lock delays until
    ...
    private function processDocument(Asset\Document $asset, ?string $processingToken): void
    {
        ...
        if ($save || $generateThumbnail) {
            $this->completeProcessing($asset, $processingToken, $save, $saveParams, $generateThumbnail);
        }
    }
    ...
    private function processVideo(Asset\Video $asset, ?string $processingToken): void
    {
        ...
        $asset->handleEmbeddedMetaData();
        $asset->setProcessingPending(false);
        if (!$this->completeProcessing($asset, $processingToken)) {
            return;
        }
    ...
    private function processImage(Asset\Image $image, ?string $processingToken): void
    {
        ...
        if (!$this->completeProcessing($image, $processingToken)) {
            return;
        }
