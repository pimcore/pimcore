...
class Ghostscript extends Adapter
{
    ...
    public function saveImage(string $imageTargetPath, int $page = 1, int $resolution = 200): bool
    {
        try {
            $localFile = self::getLocalFileFromStream($this->getPdf());
            $cmd = [self::getGhostscriptCli(), '-sDEVICE=pngalpha', '-dUseCropBox', '-dFirstPage=' . $page, '-dLastPage='
            . $page, '-dTextAlphaBits=4', '-dGraphicsAlphaBits=4', '-r'. $resolution, '-o', $imageTargetPath, $localFile];
