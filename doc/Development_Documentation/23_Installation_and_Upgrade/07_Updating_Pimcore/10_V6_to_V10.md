...
- Rename `config/routing.yaml` to `config/routes.yaml`
...
Due to change in Pimcore config that no extra keys are allowed under pimcore node anymore, it is required to adapt system settings as per [system.template.yaml](https://github.com/pimcore/demo/blob/10.x/var/config/system.template.yaml) & [config.yaml](https://github.com/pimcore/demo/blob/10.x/config/config.yaml)
