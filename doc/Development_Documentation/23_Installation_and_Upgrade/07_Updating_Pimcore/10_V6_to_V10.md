...
### Migrate Document versions to be compatible with Pimcore X
If you wish to carry forward Document version files from 6.9.x to Pimcore X, then it is required run command `pimcore:documents:migrate-elements` to migrate deprecated property `elements` to `editables`.
