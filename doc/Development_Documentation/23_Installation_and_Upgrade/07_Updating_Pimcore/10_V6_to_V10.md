...
- Move `security.firewalls` configs from bundles to `config/packages/security.yaml` as it is not allowed anymore to have firewall configuration in different files. Check `bin/console debug:config security firewalls` to find firewall configurations which have to be moved
