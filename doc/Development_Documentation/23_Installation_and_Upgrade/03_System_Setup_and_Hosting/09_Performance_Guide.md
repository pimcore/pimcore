...
When developing a high traffic website, it is a common practice to focus on performance measures and incorporate changes to make the website even more performant. A highly optimized website drive more traffic and increase the customer base.
...
By default, JIT compiler is turned off in PHP 8 and to enable it, add these configuration to ini configuration:
...
### Symfony Performance Best Practice
...
Pimcore uses extensively caches for differently types of data. The primary cache is a pure object cache where every element (document, asset, object) in Pimcore is cached as it is (serialized objects).
...
To enable automatic static page generation for a document, go to Document -> Settings -> Satic Page Generator. Mark enable checkbox and define optional lifetime for static pages (which regenerates static page after lifetime) and save document.
