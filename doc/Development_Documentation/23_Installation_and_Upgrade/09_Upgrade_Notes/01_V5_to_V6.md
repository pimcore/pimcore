...
Database settings were moved to doctrine bundle's default configuration structure (Symfony config-tree), so it is important to create a `database.yml` manually before running the update, see also  [#3268](https://github.com/pimcore/pimcore/issues/3268) 
...
doctrine:
    dbal:
        connections:
            default:
                user: username #from system.php ['database']['params']['username']
                password: password #from system.php ['database']['params']['password']
                dbname: dbname #from system.php ['database']['params']['dbname']
                host: host #from system.php ['database']['params']['host']
                port: 3306 #from system.php ['database']['params']['port']
````
...
Pimcore\Tool::checkExecutingUser()
