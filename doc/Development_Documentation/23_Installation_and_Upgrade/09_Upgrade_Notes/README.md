...
- [Document] Deprecated loading documents via fixed namespace only and will be removed in Pimcore 11.
  - Enabling custom document types. A new Typeloader is introduced, but _disabled_ by default. Enable the loader via `type_definitions_loader_enabled: true`. You can use the `type_definitions` to load and override custom document types.
## 10.5.13
