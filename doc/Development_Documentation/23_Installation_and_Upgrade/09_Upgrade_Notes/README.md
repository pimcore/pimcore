...
- [Maintenance] Image Optimizer maintenance task moved to Messages which are consumed by Symfony Messenger. If you want to exclude these jobs on maintenance execution, please add config to avoid dispatching the messages on messenger bus:
```yaml
pimcore:
    maintenance:
        exclude_messages:
