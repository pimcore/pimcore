...
With this service every image which is stored as an asset can be transformed. Pimcore does not support modifying images
that are not stored as an asset inside Pimcore.
...
Click on _+_ to add a new transformation, so that it looks like this for example:
...
Due to licensing issues Pimcore doesn't include the color profiles (\*.icc files) in the download package, but
...
Pimcore auto-generates a thumbnail if requested but doesn't exist on the file system and is directly called via its file path (not using any of
...
The structure of the path is identical to the one generated by the `getThumbnail()` methods, so it doesn't
...
combination with Web2Print documents (browser preview with normal size, tripled in size for PDF output).
...
If you're using media queries in your thumbnail configuration Pimcore automatically generates a `<picture>` tag when calling `$asset->getThumbnail("example")->getHtml()`.
...
Besides embedding thumbnails into CMS pages and distributing them via other channels, users can download a thumbnail of an asset in the Pimcore backend.
To make a thumbnail downloadable, mark "List as option in download section on image detail view" in the Image Thumbnail Advanced settings. All thumbnails with this option enabled are listed in the "Download Thumbnail" dropdown on the detail view of an Asset. To download the thumbnail, choose the thumbnail from the list and hit the "Download" button.
