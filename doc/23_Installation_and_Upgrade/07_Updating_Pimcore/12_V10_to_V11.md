...
- Update `config/packages/security.yaml` to the latest format. As reference use [security.yaml](https://github.com/pimcore/skeleton/blob/2025.x/config/packages/security.yaml) of our skeleton. 
...
With Pimcore 11, certain Symfony Messenger tasks were moved to a separate receiver. Make sure you adapt your Symfony Messenger worker configuration (e.g. your supervisor configuration [here](https://github.com/pimcore/skeleton/blob/2025.x/.docker/supervisord.conf) so that all receivers are consumed. Newly added consumers are: 
