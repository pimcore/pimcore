...
```
Update composer.json as per [skeleton](https://github.com/pimcore/skeleton/blob/11.x/composer.json)
...
Add [.env](https://github.com/pimcore/skeleton/blob/11.x/.env) file to project root, if not exists already.
...
Due to change in Pimcore config that no extra keys are allowed under pimcore node anymore, it is required to adapt system settings as per [system.template.yaml](https://github.com/pimcore/demo/blob/11.x/var/config/system.template.yaml) (default path `<Pimcore root>/var/config/system.yaml`) and [config.yaml](https://github.com/pimcore/demo/blob/11.x/config/config.yaml) (default path after adapting folder structure `<Pimcore root>/config/config.yaml`)
...
It is not longer possible to merge security configurations from multiple locations, including bundles. Instead you have to move them to one single config file, e.g. `config/packages/security.yaml`. Check `bin/console debug:config security firewalls` to find firewall configurations which have to be moved. Please adapt `config/packages/security.yaml` from [skeleton](https://github.com/pimcore/skeleton/blob/11.x/config/packages/security.yaml) and merge your own firewall configs into one single file.
