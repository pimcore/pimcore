...
Due to change in Pimcore config that no extra keys are allowed under pimcore node anymore, it is required to adapt system settings as per [system.template.yaml](https://github.com/pimcore/demo/blob/10.3/var/config/system.template.yml) (default path `<Pimcore root>/var/config/system.yaml`) and [config.yaml](https://github.com/pimcore/demo/blob/v10.3.5/config/config.yaml) (default path after adapting folder structure `<Pimcore root>/config/config.yaml`)
