...
You can enforce certain required configs by placing a `pimcore.cnf` file with the following contents into the config directory (e.g. `/etc/mysql/conf.d/`). 
...
To fix this, place the `pimcore.cnf` file in the config directory, remove the MySQL data directory and run `mysqld --initialize`. 
