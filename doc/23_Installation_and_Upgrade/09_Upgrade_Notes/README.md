...
-   Whenever any columns are denied, the report's query is also validated against its actual resolved result columns (not just the configured fragment text) - so an empty/omitted `sql` fragment, a wildcard column selection (`*`, `alias.*`, `DISTINCT *`), or a denied column surfaced through a subquery is rejected *if it would actually expose a denied column*, even though none of these name the denied column directly in the configuration. Aggregate functions like `COUNT(*)` are unaffected. `UNION`, `INTERSECT` and `EXCEPT` are not supported in Custom Report SQL fragments and are always rejected, regardless of the deny-list configuration.
-   Whenever either deny-list is non-empty, reports are also rejected outright (independent of which tables/columns they reference) if the database connection's session `sql_mode` includes `ANSI_QUOTES` or `NO_BACKSLASH_ESCAPES` - these change how the connection itself parses quoted identifiers/string literals in a way this deny-list's own parsing can't safely account for.

**Action Required:**
If you have existing SQL Custom Reports that legitimately reference the `users` table, columns named `password`, `passwordRecoveryToken`, `twoFactorAuthentication`, `apiKey`, `secret` or `token` (on any table), rely on an empty `sql` fragment/wildcard column selection that resolves to one of those columns, or use `UNION`/`INTERSECT`/`EXCEPT`, they will now be rejected with an `InvalidArgumentException`. Adjust the deny-list to fit your schema via the standard `pimcore_custom_reports` bundle configuration, and/or list the exact columns your report needs instead of relying on a wildcard. If your database connection's session uses `ANSI_QUOTES` or `NO_BACKSLASH_ESCAPES` and you rely on either deny-list, every SQL Custom Report will be rejected until you either disable that `sql_mode` for the connection or clear both `denied_tables` and `denied_columns` (accepting that the deny-list protection is then off):
