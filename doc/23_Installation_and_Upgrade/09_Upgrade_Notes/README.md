...
-   Whenever any columns are denied, the report's query is also validated against its actual resolved result columns (not just the configured fragment text) - so an empty/omitted `sql` fragment, a wildcard column selection (`*`, `alias.*`, `DISTINCT *`), or a denied column surfaced through a subquery is rejected *if it would actually expose a denied column*, even though none of these name the denied column directly in the configuration. Aggregate functions like `COUNT(*)` are unaffected. `UNION` is not supported in Custom Report SQL fragments and is always rejected, regardless of the deny-list configuration.

**Action Required:**
If you have existing SQL Custom Reports that legitimately reference the `users` table, columns named `password`, `passwordRecoveryToken`, `twoFactorAuthentication`, `apiKey`, `secret` or `token` (on any table), rely on an empty `sql` fragment/wildcard column selection that resolves to one of those columns, or use `UNION`, they will now be rejected with an `InvalidArgumentException`. Adjust the deny-list to fit your schema via the standard `pimcore_custom_reports` bundle configuration, and/or list the exact columns your report needs instead of relying on a wildcard:
