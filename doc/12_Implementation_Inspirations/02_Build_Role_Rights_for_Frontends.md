...
One way of achieving this is to leverage Symfony Security functionality with access control configuration
...
So how to integrate Pimcore objects, a roles & rights system and Symfony best practices to implement complex permission
...
See the [Pimcore docs](../09_Development_Tools/03_Security_and_Authentication/01_Authenticate_Pimcore_Objects.md) 
...
A `UserRole` might look similar to the following example and contain `permissionResources` for itself. 
![User Role Class](img/user-role-class.jpg)

The userRole objects can have `permissionResources` with the name like these in the user object (in this case values are 
merged as described below), or can have additional `permissionResources`.  
...
Complex permission structures can be set up with Pimcore objects. 
...
of this user. In this case you can access this information everywhere in the Symfony framework. For details see docs in the
