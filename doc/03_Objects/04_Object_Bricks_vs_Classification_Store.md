...
| Access to the data via API | Quite simply via getter and setter as known from Pimcore objects - see [here](./01_Object_Classes/01_Data_Types/60_Object_Bricks.md#working-with-php-api) | Slightly harder on generic getter / setter calls - see [here](./01_Object_Classes/01_Data_Types/15_Classification_Store.md#using-classification-store-via-php-api)
| Filter in object listings | Using JOINs directly in the Object-Listing Query, see [here](./01_Object_Classes/01_Data_Types/60_Object_Bricks.md#querying-for-objectbrick-data) | Only possible via custom subqueries and rather tedious.
...
There is no single rule that always applies. The right choice depends on many factors, and sometimes a combination of both is the best approach. The decision should be made by a developer based on the specific project requirements.
