---
title: Custom Icons and Tooltips
description: Override element tree icons and tooltips via Studio Backend event subscribers.
---

# Custom Icons and Tooltips in the Element Tree and Editor Tabs

Customize how elements appear in the **tree** and **editor tabs** by overriding their **icon** and **tooltip** via
backend event subscribers. The Studio UI renders these customizations automatically; no frontend plugin code is needed for basic cases.
...
Subscribe to `pre_response.*` events to modify the `CustomAttributes` before the response is sent to the frontend. 
Each element type has its own event:
...
> **Important:** To customize the icon everywhere (tree **and** editor tabs), subscribe to **both** the tree event and 
> the detail event. The tree endpoint and the detail/editor endpoint fire separate events.

### Example: Custom Icon and Tooltip for Data Objects (Tree and Editor Tabs)

This subscriber sets a flag icon and an HTML tooltip for all objects of class "Demo", in both the tree 
and the editor tab:
