...
### [Workflow]
- [Marking Stores] The `state_table` marking store no longer writes the new place to `element_workflow_state` right away when a transition with `changePublishedState: save_version` is applied through the workflow manager (`Manager::applyWithAdditionalData()` with `$saveSubject = true`, which is what the admin UIs use). The place is instead kept pending on the element and stored with the draft version, and it is written to the table once the element is fully saved (e.g. when the draft is published). Discarding the draft now discards the place change as well, consistent with the attribute based marking stores (`single_state`, ...). See [#18653](https://github.com/pimcore/pimcore/issues/18653). Consumers reading `element_workflow_state` directly therefore see the state of the published element while such a draft exists. Transitions with any other `changePublishedState`, global actions and direct `$workflow->apply()` calls keep persisting the place immediately. Note for Pimcore Studio: Studio currently applies transitions to the published element instead of the latest draft (see [pimcore/platform-version#455](https://github.com/pimcore/platform-version/issues/455)); until that is fixed, the pending place is stored with the version the transition creates, but the user's earlier draft edits are not part of it.
- Added `Pimcore\Workflow\MarkingStore\PendingMarkingStoreInterface` for marking stores that persist the marking independently of the subject and want to support the behavior above; `StateTableMarkingStore` implements it. `Pimcore\Model\Element\AbstractElement` got the `@internal` methods `getPendingWorkflowMarking()`, `setPendingWorkflowMarking()` and `getPendingWorkflowMarkings()` which carry such a pending marking; the value is part of version dumps but excluded from the cache.
