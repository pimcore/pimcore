...
- [Commands] The table output of `pimcore:bundle:list` uses the narrow `I?` / `UI?` column headers (with an explaining legend) again instead of `Installable` / `Uninstallable`. The `--json` output keeps the full key names; the only change there is that with `--details` the description is now truncated to 30 characters including the `...` ellipsis (previously up to 33 characters), which also applies to the table output.
