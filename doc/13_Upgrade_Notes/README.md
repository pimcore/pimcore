...
- [Thumbnails] A `cover` transformation now applies the asset's focal point even when the transformation has no `positioning` argument configured at all. Previously the focal point was only injected while iterating the *configured* arguments, so it was silently dropped for thumbnail definitions that omit `positioning` (common in hand-written config files) and the image was cropped centered instead. Both focal point coordinates are now required and `0` is treated as a valid coordinate (left/top edge) rather than as "no focal point"; the new `Pimcore\Model\Asset\Image::getFocalPoint()` is the single source of truth for that rule and is used by both the thumbnail processor and the CDN URL listener. **Operational note:** the name of a generated rendition is derived from the thumbnail configuration and the asset checksum only, so renditions that were generated before this fix stay valid cache hits and keep their centered crop. Installations with focal points on already-thumbnailed assets have to clear the affected renditions once after the upgrade, e.g. with `bin/console pimcore:thumbnails:clear`.
