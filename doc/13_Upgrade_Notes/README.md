...
### [DataObject]
- [Relations] Two new asset-only relation data types are available: `manyToManyAssetRelation` (`Pimcore\Model\DataObject\ClassDefinition\Data\ManyToManyAssetRelation`, extends `ManyToManyRelation`) and `advancedManyToManyAssetRelation` (`Pimcore\Model\DataObject\ClassDefinition\Data\AdvancedManyToManyAssetRelation`, extends `AdvancedManyToManyRelation`). Both only accept assets (documents and objects cannot be enabled), support read-only *visible fields* resolved from asset properties or asset metadata, and the advanced variant stores metadata columns per relation in `object_metadata_*` as `AdvancedManyToManyRelation` does. Their query-table column holds plain asset IDs (`,350,351,`) instead of the `type|id` pairs written by `ManyToManyRelation`. Existing fields are not affected; see [Relational Datatypes](../03_Objects/01_Object_Classes/01_Data_Types/70_Relation_Types.md#many-to-many-asset-relation).
