...
- [Relations] `manyToManyRelation` and `advancedManyToManyRelation` now support **visible fields**, the read-only extra columns previously only available on the object-specific relation types. The offered fields are the union of what every allowed element type provides (class fields of the allowed object classes including localized ones, `filename` / `mimetype` / `fileSize` and predefined metadata for assets, `creationDate` / `modificationDate` for all types); a field that does not apply to a related element's type is empty for that row. `ManyToManyRelation` now implements `LayoutDefinitionEnrichmentInterface` and exposes `setVisibleFields()` / `getVisibleFields()` / `getVisibleFieldNames()`, `getAvailableVisibleFields()`, `enrichLayoutDefinition()` (fills the new public `visibleFieldDefinitions` property) and `getVisibleFieldData()`; the shared implementation lives in the `@internal` `Pimcore\Model\DataObject\ClassDefinition\Data\Relations\VisibleFieldsTrait`. `AdvancedManyToManyRelation::getDataForEditmode()` adds the visible-field values to each row (metadata columns of the same name take precedence); the row shape of `ManyToManyRelation::getDataForEditmode()` is unchanged. Custom subclasses that already declare a `visibleFields` / `visibleFieldDefinitions` property or an `enrichLayoutDefinition()` method must be reconciled with the new trait. See [Relational Datatypes](../03_Objects/01_Object_Classes/01_Data_Types/70_Relation_Types.md#visible-fields-on-many-to-many-and-advanced-many-to-many-relations).
- [Relations] The visible-field definitions of `manyToManyObjectRelation` and `advancedManyToManyObjectRelation` (`visibleFieldDefinitions`, filled by `enrichLayoutDefinition()`) are now built by the shared `@internal` helper `Pimcore\Model\DataObject\ClassDefinition\Data\Relations\VisibleFieldDefinitionHelper`. The two previously separate implementations were merged: `AdvancedManyToManyObjectRelation` no longer overrides `enrichLayoutDefinition()` but only the new `protected` `getVisibleFieldsClassIdentifier()` (returning its `allowedClassId`; the base type returns the first entry of `classes`). The only output change: a visible field name that does not resolve to a field of the class (which is still rendered as a plain `input` column) now carries `noteditable: true` like resolved fields do. Subclasses overriding `enrichLayoutDefinition()` on these types should switch to overriding `getVisibleFieldsClassIdentifier()` where they only change the class lookup.
