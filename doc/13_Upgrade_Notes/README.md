...
### [Database]
- Added `Pimcore\Db\Helper::upsertByUniqueKey()`, which inserts or updates a row with a single `INSERT ... ON DUPLICATE KEY UPDATE` statement instead of the `INSERT` plus `UPDATE ... WHERE $keys` on a duplicate that `Helper::upsert()` runs. The core DAOs now use it for every table whose primary key is its only unique index (the class store, query and localized tables, properties, versions, notes and so on); the `objects`, `assets` and `documents` tables, whose `fullpath` index is a second unique index, use the new `Pimcore\Db\Helper::updateOrInsert()` instead, which has exactly the contract of `Helper::upsert()` (an `UPDATE ... WHERE $keys`, no other row is ever touched) but runs the `UPDATE` first and only falls back to `Helper::upsert()` when it changed no row - a single statement for the usual save of an existing element. `Helper::upsert()` itself is unchanged. The new method has the same return value (last insert id on insert, `null` on update) but requires the key columns to be the primary key or a unique index of the table; see its docblock for the details of the contract (only the conflicting row is ever touched, and on a table with another unique index a conflict on that index leaves the row unchanged but still runs its `UPDATE` triggers with `NEW` equal to `OLD`, so a `BEFORE UPDATE` trigger that assigns to `NEW` does write to it, where `Helper::upsert()` runs no trigger at all). Calls with an empty key list, a key column missing from the data or a connection with `CLIENT_FOUND_ROWS` semantics (`PDO::MYSQL_ATTR_FOUND_ROWS`, or `MYSQLI_CLIENT_FOUND_ROWS` in the mysqli `flags`) fall back to `Helper::upsert()`.
