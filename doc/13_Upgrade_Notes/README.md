...
### [Assets]
- [Thumbnails] The public thumbnail route now returns a `404 Not Found` when the requested asset or thumbnail configuration does not exist, or when the thumbnail can neither be delivered nor (re)generated — previously these cases redirected to the placeholder image. The placeholder redirect is now reserved for thumbnails whose generation actually failed (e.g. a broken source file). A thumbnail file merely missing from the thumbnail storage is still regenerated on demand as before; additionally, a stale thumbnail status cache entry claiming such a missing file exists is now invalidated when a read runs into it (previously this caused a repeated unhandled `League\Flysystem\UnableToReadFile`, i.e. a 500), so the thumbnail is regenerated on the next access. A read failure for a thumbnail file that still exists on the storage — or whose existence cannot be determined — is surfaced as the underlying storage error instead of being masked as a cache miss and silently triggering a regeneration on every request.
