...
- [Relations] `manyToManyRelation` and `advancedManyToManyRelation` now support **visible fields**, the read-only extra columns previously only available on the object-specific relation types. The offered fields are the union of what every allowed element type provides (class fields of the allowed object classes including localized ones, except container fields and the secret-holding `password` / `encryptedField`; `filename` / `mimetype` / `fileSize` and predefined metadata for assets, `creationDate` / `modificationDate` for all types); a field that does not apply to a related element's type is empty for that row. `ManyToManyRelation` now implements `LayoutDefinitionEnrichmentInterface` and exposes `setVisibleFields()` / `getVisibleFields()` / `getVisibleFieldNames()`, `getAvailableVisibleFields()`, `enrichLayoutDefinition()` (fills the new public `visibleFieldDefinitions` property) and `getVisibleFieldData()`; the shared implementation lives in the `@internal` `Pimcore\Model\DataObject\ClassDefinition\Data\Relations\VisibleFieldsTrait`. The edit-mode rows of both types are unchanged; the values are resolved by the consumer per row through `getVisibleFieldData()`. Custom subclasses that already declare a `visibleFields` / `visibleFieldDefinitions` property or an `enrichLayoutDefinition()` method must be reconciled with the new trait. See [Relational Datatypes](../03_Objects/01_Object_Classes/01_Data_Types/70_Relation_Types.md#visible-fields-on-many-to-many-and-advanced-many-to-many-relations).
