...
### [Database]
- `Pimcore\Db\Helper::upsert()` now runs a single `INSERT ... ON DUPLICATE KEY UPDATE` statement instead of an `INSERT` followed by an `UPDATE ... WHERE $keys` on a duplicate. The return value (last insert id on insert, `null` on update) and the core call sites are unchanged, and calls with an empty `$keys` list, a key column missing from `$data` or a connection with `PDO::MYSQL_ATTR_FOUND_ROWS` enabled keep the previous two-statement behavior. Two contract points are now explicit for other callers of this public helper:
  - `$keys` must be the primary key or a unique index of the table. `ON DUPLICATE KEY UPDATE` can only touch the row the conflict was detected on, so non-unique criteria update that row if it matches and nothing else, where the previous `UPDATE ... WHERE $keys` addressed every matching row (and, as it wrote the conflicting unique value to all of them, failed with a unique constraint violation whenever the criteria matched any row but the conflicting one).
  - On a conflict with a different unique index than `$keys` (e.g. the unique path index of `objects`, `assets` or `documents`), the conflicting row is left unchanged and `null` is returned as before, but the database still runs that row's `BEFORE UPDATE` triggers with `NEW` equal to `OLD`; the previous `UPDATE` matched no row there and ran no trigger.
