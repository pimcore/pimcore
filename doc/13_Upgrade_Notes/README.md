...
### [Assets]
- [CLI] The command `pimcore:assets:add-to-update-task-queue` supports a new option `--missing-metadata` (`-m`), which restricts the assets that are added to the update task queue to those that are missing the meta-data written by the asset update task (embedded meta-data and dimensions for images, embedded meta-data, duration and dimensions for videos, page count for documents). This allows backfilling assets that were uploaded before the `pimcore_asset_update` queue was consumed, without re-processing all assets. Documents are only considered if `pimcore.assets.document.process_page_count` is enabled, and assets that are flagged as failed are skipped unless `--retry-failed` is used as well. The command now also outputs the number of assets that are added to the queue, and with `-v` the individual assets.
- [CLI] The `--retry-failed` option of `pimcore:assets:add-to-update-task-queue` matched the `pimcore-asset-processing-failed` custom setting with a `LIKE` on the serialized `customSettings` column. As MySQL renders JSON columns with a space after the colon (`{"key": true}`) while MariaDB stores the value verbatim, the option silently matched nothing on MySQL. It now uses `JSON_EXTRACT()`, which behaves the same on both.
