...
- Added two methods next to `Pimcore\Db\Helper::upsert()`, which itself is unchanged. Both have its return value (last insert id on insert, `null` on update) and the core DAOs now use them; an upgrade needs no changes.
  ...
  - `Helper::updateOrInsert()` addresses exactly the rows `upsert()` does (an `UPDATE ... WHERE $keys`, no other row is ever touched, no trigger runs on a row the criteria do not match) but runs the `UPDATE` first; if it changes no row, a `SELECT` on the same criteria tells an unchanged row from a missing one, and only a missing row goes through `upsert()`. The core uses it where the row exists at save time and the table can have a second unique index: `objects`, `assets` and `documents` (`fullpath`), `tags` (`idPath`, `name`), and the class store and localized store tables on an update (a field marked unique adds a unique index there). The one observable difference to `upsert()`: its `INSERT`, failing on the duplicate, ran the table's `BEFORE INSERT` triggers on the update path too (rolled back with the failed statement); `updateOrInsert()` runs them only when it actually inserts.
