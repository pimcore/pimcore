...
### [Assets]
- [Versions] New opt-in config option `pimcore.assets.versions.skip_initial_version` (default `false`): when enabled, no version is created when an asset is added (uploaded). Instead, the persisted state of the asset is versioned lazily right before its first modification is applied, followed by the regular version of the modification, so the upload state of edited assets remains restorable while assets that are never modified don't occupy any version storage. Folders are not affected (they are never versioned), `Asset::saveVersion()` called directly still always creates a version. The `@internal` helpers `Asset::isInitialVersionSkipped()`, `Asset::isVersionCreationEnabledByConfig()`, `Asset::saveVersionOfPersistedState()` and `Asset\Dao::hasVersionsForUpdate()` were added. See [Versioning](../05_Content_Management_Features/01_Versioning.md#skip-the-initial-asset-version).
