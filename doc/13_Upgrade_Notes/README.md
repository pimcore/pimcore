...
### [DataObject]
- [Relations] A new asset-only relation data type is available: `manyToManyAssetRelation` (`Pimcore\Model\DataObject\ClassDefinition\Data\ManyToManyAssetRelation`, extends `ManyToManyRelation`). It only accepts assets (documents and objects cannot be enabled) and supports read-only *visible fields* resolved from asset metadata. Its query-table column holds plain asset IDs (`,350,351,`) instead of the `type|id` pairs written by `ManyToManyRelation`. Existing fields are not affected; see [Relational Datatypes](../03_Objects/01_Object_Classes/01_Data_Types/70_Relation_Types.md#many-to-many-asset-relation).
