...
### [Workflows]
- [GlobalActions] Global actions now support `changePublishedState`, `unsavedChangesBehaviour` and `notificationSettings`, which so far were only available on transitions. They are configured directly on the global action (global actions have no `options` sub-key) and behave exactly like their transition counterparts. Defaults (`no_change`, `warn` and no notifications) preserve the previous behavior, so no configuration change is required.
  - `changePublishedState` is applied by `Pimcore\Workflow\EventSubscriber\ChangePublishedStateSubscriber`, which now also listens to `Pimcore\Event\WorkflowEvents::POST_GLOBAL_ACTION`. `save_version` only takes effect if `saveSubject` is enabled, since it replaces the regular save of the subject with `saveVersion()`.
  - `notificationSettings` are sent by `Pimcore\Workflow\EventSubscriber\NotificationSubscriber`, which now also listens to `Pimcore\Event\WorkflowEvents::POST_GLOBAL_ACTION`.
  - As a consequence, `NotificationEmailService::sendWorkflowEmailNotification()` and `PimcoreNotificationService::sendPimcoreNotification()` accept `Pimcore\Workflow\Transition|Pimcore\Workflow\GlobalAction` instead of only `Pimcore\Workflow\Transition` for their `$transition` argument. If you override either method in a subclass, widen its parameter type accordingly.
