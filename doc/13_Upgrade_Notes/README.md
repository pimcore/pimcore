...
### [Database]
- Added `Pimcore\Db\Helper::upsertByUniqueKey()`, which inserts or updates a row with a single `INSERT ... ON DUPLICATE KEY UPDATE` statement instead of the `INSERT` plus `UPDATE ... WHERE $keys` on a duplicate that `Helper::upsert()` runs. All core DAOs now use it. `Helper::upsert()` itself is unchanged. The new method has the same return value (last insert id on insert, `null` on update) but requires the key columns to be the primary key or a unique index of the table; see its docblock for the details of the contract (only the conflicting row is ever touched, and on a conflict with a different unique index the statement leaves that row unchanged but its `UPDATE` triggers still run with `NEW` equal to `OLD`, so a `BEFORE UPDATE` trigger that assigns to `NEW` does write to it, where `Helper::upsert()` ran no trigger at all). Calls with an empty key list, a key column missing from the data or a connection with `PDO::MYSQL_ATTR_FOUND_ROWS` enabled fall back to `Helper::upsert()`.
