...
### [Assets]
- [Thumbnails] The public thumbnail route now returns a `404 Not Found` when the requested asset, thumbnail configuration or thumbnail file does not exist, instead of redirecting to the placeholder image (the placeholder redirect is now reserved for thumbnails whose generation failed). A stale thumbnail status cache entry pointing to a file missing from the thumbnail storage is invalidated, so the thumbnail is regenerated on the next request instead of repeatedly failing. A read failure for a thumbnail file that still exists on the storage is surfaced as the underlying storage error instead of silently triggering a regeneration on every request.
