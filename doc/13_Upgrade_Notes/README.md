...
### [Database]
- Added two methods next to `Pimcore\Db\Helper::upsert()`, which itself is unchanged. Both have its return value (last insert id on insert, `null` on update) and the core DAOs now use them; an upgrade needs no changes.
  - `Helper::upsertByUniqueKey()` inserts or updates a row with a single `INSERT ... ON DUPLICATE KEY UPDATE` instead of the `INSERT` plus `UPDATE ... WHERE $keys` on a duplicate that `upsert()` runs. It requires the key columns to be the primary key or a unique index of the table and is meant for tables where that is the only unique index the data can collide on; the core uses it for the classification store tables, `properties`, `versions`, `notes` and the other fixed-schema tables with a single unique index. See its docblock for the contract (only the conflicting row is ever touched; a conflict on another unique index leaves that row unchanged but still runs its `UPDATE` triggers with `NEW` equal to `OLD`, so a `BEFORE UPDATE` trigger that assigns to `NEW` does write to it; and on the update path the table's `BEFORE INSERT` triggers run and their effects persist, including an assignment to `NEW`, where `upsert()`'s failing `INSERT` rolled them back). Calls with an empty key list, a key column missing from the data or null, or a connection with `CLIENT_FOUND_ROWS` semantics (`PDO::MYSQL_ATTR_FOUND_ROWS`, or `MYSQLI_CLIENT_FOUND_ROWS` in the mysqli `flags`) fall back to `upsert()`.
  - `Helper::updateOrInsert()` addresses exactly the rows `upsert()` does (an `UPDATE ... WHERE $keys`, no other row is ever touched, no trigger runs on a row the criteria do not match) but runs the `UPDATE` first. That `UPDATE` also records whether it matched a row (through `LAST_INSERT_ID(<random token>)` in one of its assignments, read back only when no row changed), so a row that matched but did not change - it already holds the values, or a `BEFORE UPDATE` trigger reset them - is done after that one statement and its triggers run once; only a missing row goes through `Helper::upsert()`, whose duplicate handling also covers a row inserted concurrently. After such an `UPDATE` the connection's last insert id holds that token. The core uses it where the row exists at save time and the table can have a second unique index: `objects`, `assets` and `documents` (`fullpath`), `tags` (`idPath`, `name`), and the class store, query, localized and brick tables on an update (a field marked unique adds a unique index to the store tables). The one observable difference to `upsert()`: on the update of a row whose values change, `upsert()`'s `INSERT` failed on the duplicate and ran the table's `BEFORE INSERT` triggers first (rolled back with the failed statement); `updateOrInsert()` runs them only when it actually tries to insert.
