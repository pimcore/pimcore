...
### [Workflow]
- [Places] `changePublishedState` can now be configured on a place in addition to a transition, so a published state can be enforced when an element enters a place instead of having to repeat the setting on every incoming transition. It applies to completed workflow transitions and to global actions which set places via their `to` option; elements which reach a place through `initial_markings` or a direct marking store write are not affected, so this is an entry behaviour rather than an invariant of being in the place. A `changePublishedState` set on the applied transition still takes precedence; if the transition leaves it at `no_change`, the setting of the places the transition leads to is used (the first place defining one, in the order of the workflow config, wins). Existing configurations are unaffected, since places default to `no_change`.
...
- The inherited value is resolved when the container is compiled, so `Pimcore\Workflow\Transition::getChangePublishedState()` and the new `Pimcore\Workflow\GlobalAction::getChangePublishedState()` report the value which actually applies, not just the one written on the transition. Consumers reading the option off a transition therefore need no changes. `Pimcore\Workflow\Place\PlaceConfig::getChangePublishedState()` returns the raw value configured on the place.
