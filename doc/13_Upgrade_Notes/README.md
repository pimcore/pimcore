...
### [Assets]
- [Documents] Document assets (e.g. PDFs) now support embedded metadata (XMP/EXIF/IPTC) extraction, analogous to image and video assets. The metadata is extracted asynchronously via the asset update tasks queue (preferably using `exiftool`, if available) and is available through `Asset\Document::getEmbeddedMetaData()` or the `embeddedMetaData` custom setting. Existing document assets are not processed automatically after the upgrade, as assets are only added to the queue when their binary data changes. To extract the embedded metadata of existing documents, add them to the update task queue, e.g. `bin/console pimcore:assets:add-to-update-task-queue --path-pattern='\.pdf$'`, and process the `pimcore_asset_update` messenger transport. The command re-processes all matching image, video and document assets (previews, dimensions, page counts, ...), which can be costly for large asset trees, so limit the scope with the `--parent`, `--id` or `--path-pattern` options where possible.
- [Embedded Metadata] The `embeddedMetaData` and `embeddedMetaDataExtracted` custom settings are now removed for all asset types as soon as new binary data is assigned to an asset (`setData()` / `setStream()`), so the metadata is extracted again from the new data. Loading or restoring a version, restoring an asset from the recycle bin and copying an asset keep the metadata. The invalidation also covers changes of the asset type (e.g. an image asset that is replaced by a PDF and therefore becomes a document asset), which previously kept the metadata of the old file.
- [Versions][Recycle Bin] Assets loaded from a version or restored from the recycle bin now keep the custom settings of the dumped state, including the settings derived from the binary data (embedded metadata, image and video dimensions, video duration, document page count and PDF scan status), which are no longer invalidated when the restored data is saved. The new `@internal` method `Asset::isDataReplaced()` tells apart replaced from restored data for this purpose. While the processing of replaced data by the asset update tasks queue is pending, this is remembered in the new custom setting `pimcore-asset-processing-pending` (see the new `@internal` method `Asset::isProcessingPending()`), so that a version or recycle bin entry created in the meantime is processed again after restoring it (the same applies to a version or recycle bin entry created directly after replacing the data, without saving the asset; such a version or entry now contains the replaced data, previously it contained the data stored before, as cloning an asset closed the stream of the new data for the original asset as well). For the same reason, copying an asset (`Asset\Service::copyAsChild()`, `copyRecursive()`, `copyContents()`) no longer assigns the stream of the source to the copy, but a new stream of the same data (see the new `@internal` method `Asset::getStreamCopy()`), so saving the copy no longer closes the stream of the source. Versions and recycle bin entries created before this change are always processed again after restoring them (as before this change), as it is unknown whether their processing was still pending. Versions and recycle bin entries created before this change from an asset whose custom settings had been omitted from the cache (as they were too large for it) don't contain any custom settings; when such a dump is loaded or restored, the custom settings are empty (they are no longer replaced by the current ones from the database), and the derived settings are generated again by the asset update tasks queue after restoring it. Previously, they were replaced by the current custom settings of the asset in the database on first access, which meant that a version showed the current custom settings and that restoring an asset from the recycle bin lost all of its custom settings (they were regenerated by the asset update tasks queue afterwards, where possible).
- [Assets] Every replacement or restore of the data of an asset now assigns a new value to the new custom setting `pimcore-asset-data-generation` (see the new `@internal` method `Asset::getDataGeneration()`), which identifies the current data (even if it is identical to the previous one). The tasks which are added to the asset update tasks queue when replaced data is saved are bound to this data: `Pimcore\Messenger\AssetUpdateTasksMessage` has the new optional constructor arguments `dataGeneration` and `previewsOnly`, and such a task is skipped if the data was replaced or restored before it is handled (as the current data is processed by its own task or doesn't need processing), or if its processing was finished in the meantime. Previously, it processed the current state of the asset, which could overwrite the derived settings of a processed state restored in the meantime. For the same reason, the results of a task are only saved if the data wasn't replaced or restored while it was processed (see the new `@internal` methods `Asset::saveProcessingResults()` and `Asset::getDataState()`, which check this within the transaction saving the asset, while the asset is locked against concurrent saves); the custom settings which don't belong to the data are saved as stored in the meantime, so changes made by others while the data was processed are kept. The previews are generated after the results were saved (without locking the asset, serialized per asset among the tasks) and cleared again if the data was changed in the meantime; a failure generating them doesn't fail the task, as the results are already saved. Restoring a processed state (e.g. from a version or the recycle bin) clears the previews of the previous data; a task bound to the restored data generates them again without processing the data. Saving an asset without changing its data keeps the fields which belong to the data (its type and mime type, the custom settings describing the state of the data, the checksum and the derived settings, see `Asset::getDataDerivedCustomSettingKeys()` of the current and the previous type) as stored in the database, if the data or its processing was changed by others since the instance was loaded: an outdated instance (even of another type, e.g. an image replaced by a document) neither discards a pending processing nor overwrites its results with outdated derived settings (the results of a processing the instance finished itself are saved), and it keeps the stored location of the asset (filename and parent), unless it changed the location on purpose (`Asset::setFilename()`, `setParentId()` and `setParent()` mark these fields as dirty now), as the data might have been renamed or moved together with the change. The results of a processing are not saved for an asset which was deleted in the meantime (this would create it again), and no previews are generated for it. While the processing of data is pending, the asset update tasks queue removes the derived settings before processing the data, so outdated ones written back in the meantime don't survive, even if the processing doesn't generate them again (e.g. because it is disabled). The marker of a failed processing (`pimcore-asset-processing-failed`) is removed whenever the data is replaced, also by data of a type which isn't processed. Copying an asset (`Asset\Service::copyAsChild()`, `copyRecursive()`, `copyContents()`) takes over the derived settings of the source (see the new `@internal` method `Asset::copyDataFrom()`): the copy is only processed if the processing of the source is pending, otherwise only its previews are generated. Tasks created by `Asset::addToUpdateTaskQueue()` and `Asset::triggerUpdateTask()`, e.g. by `pimcore:assets:add-to-update-task-queue`, are not bound to the data and process the asset in any case, as before. `Asset\Image::update()` and `Asset\Video::update()` now clear the thumbnail files after `parent::update()` (which locks the asset) instead of before (also if saving fails, as the new data might already have been written to the storage), so that a preview of the previous data generated by a task in the meantime doesn't survive the change; the thumbnail custom settings of videos are still cleared before; `Asset\Document::update()` clears the thumbnails also if saving fails. A version created by saving such an outdated instance is dumped from an instance of the class of the current type (see the new `` method `Element\AbstractElement::getDataForVersion()`), loaded from the database; the same applies if the data was replaced by data of the same type by others since the instance was loaded, as the instance can still hold the previous data. Restored data keeps the checksum dumped with it (unless it is missing), only the checksum of replaced data is generated again. The size limit of XMP packets read by `EmbeddedMetaDataTrait::getXMPData()` is enforced exactly, a packet whose close tag lies beyond the limit is rejected. `Pimcore\Helper\TemporaryFileHelperTrait::getLocalFileFromStream()` now copies the data of all streams of the `php://` wrapper (e.g. `php://memory`) to a temporary file, not only those of `php://temp`, as their URI can't be opened again with the same data. `Asset::getStreamCopy()` throws an exception if the new stream can't be opened, so a copy of an asset fails instead of being saved without the data. Every finished processing assigns a new value to the new custom setting `pimcore-asset-processing-revision`, which is part of the state of the data, so an instance loaded before already processed data was processed again on demand doesn\x27t overwrite the new results either. A copy of an asset whose data was replaced without saving it is processed like the replacement. Exceptions implementing the new `@internal` interface `Pimcore\Model\Exception\SaveAbortedExceptionInterface` abort saving an element on purpose: they are neither logged as critical nor dispatched as failure events.
