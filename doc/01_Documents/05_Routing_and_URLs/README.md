...
Pimcore defines a few system routes that are required for Pimcore base functionality like the admin user interface, and also routes provided by custom bundles. These routes are just standard Symfony routes and have the highest 
priority. 
...
Redirects with priority 99 come second in the processing priority. See [Redirects](./02_Redirects.md) for details. 
...
With the special data type [URL Slug](../../03_Objects/01_Object_Classes/01_Data_Types/65_Others.md#url-slug) URLs for 
data objects can be defined. These need to be unique and are evaluated in the fourth priority for the current site. 
See [URL Slug](../../03_Objects/01_Object_Classes/01_Data_Types/65_Others.md#url-slug) for details.
...
When your application has functionality where there is no Pimcore Document necessary (e.g. product lists, detail pages, 
 ...
 controllers. Custom Routes come fifth in the processing priority. See [Custom Routes](../../backlog/02_MVC_Custom_Routes.md) for details.


#### 6. Redirects: 
All Redirects with priority lower than 99 come sixth in the processing priority and once again processed based on their configured priority. 
