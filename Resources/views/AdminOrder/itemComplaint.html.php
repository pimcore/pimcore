...
<form action="<?= $urlSave ?>" method="post">
    <div class="modal-header">
        ...
        <h4 class="modal-title"><?= $orderItem->getProductName() ?> <small><?= $this->translateAdmin('online-shop.back-office.order.item-complaint') ?></small></h4>
    </div>
    <div class="modal-body">
        ...
        <div class="row form-group">
            <label class="col-sm-12" for="message"><?= $this->translateAdmin('online-shop.back-office.order.item-complaint.message') ?></label>
            <div class="col-sm-12"><textarea class="form-control" name="message" id="message" rows="8"></textarea></div>
        </div>

    </div>
    <div class="modal-footer">
        <button type="submit" class="btn btn-primary" name="confirmed" value="1"><?= $this->translateAdmin('online-shop.back-office.order.item-complaint') ?></button>
    </div>
</form>
