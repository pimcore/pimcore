...
Ext.onReady(function () {
    ...
    Ext.Ajax.on('requestexception', function (conn, response, options) {
        ...
        xhrActive--;
        if (xhrActive < 1) {
            Ext.get("pimcore_logo").dom.innerHTML = '<img class="logo" src="/pimcore/static/legacy/img/logo.png"/>';
        }
    });
    Ext.Ajax.on("beforerequest", function () {
        if (xhrActive < 1) {
            Ext.get("pimcore_logo").dom.innerHTML = '<img class="activity" src="/pimcore/static/legacy/img/loading.gif"/>';
        }
        xhrActive++;
    });
