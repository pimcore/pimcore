...
class WebsiteSetting_Resource extends Pimcore_Model_Resource_Abstract {
    ...
    public function getByName($name = null, $siteId = null) {
        ...
        $data = $this->db->fetchRow("SELECT id FROM website_settings WHERE name = ? AND (siteId IS NULL OR siteId = '' OR siteId = ?) ORDER BY siteId DESC", array($this->model->getName(), $siteId));
