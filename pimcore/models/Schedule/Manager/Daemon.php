...
class Schedule_Manager_Daemon extends Schedule_Manager_Procedural {
    ...
    public function __construct($pidFileName, $waitForChildrenToFinish = true) {
        ...
        if(!function_exists("pcntl_fork") or !function_exists("pcntl_waitpid") or !function_exists("pcntl_wexitstatus") or !function_exists("pcntl_signal")){
            ...
            throw new Exception("pcntnl not available. Cannot create ".get_class($this));
        }
     ...
     */
    public function run() {
        Logger::info(get_class($this) . ": Running with PID " . $this->parentPID);
        ...
        if($this->waitForChildrenToFinish){
            ...
            Logger::info(get_class($this).": finished Jobs, all child threads are done.");
        } else {
            Logger::info(get_class($this).": spawned all children, not waiting for them to finish.");   
        }
    ...
    protected function launchJob($job) {
        ...
        if ($pid == -1) {
            ...
            Logger::error(get_class($this) . ': Could not launch new job with id [ ' . $job->getId() . ' ], exiting');
            return false;
        }
        else if ($pid) {
          ...
          if (isset($this->signalQueue[$pid])) {
            ...
            try {
                ...
                Logger::error($e);
                Logger::error(get_class($this) . ": Failed to execute job with id [ " . $job->getId() . " ] and method [ ".$job->getMethod()." ]");
            }
            $job->unlock();
            Logger::debug(get_class($this) . ": Done with job [ " . $job->getId() . " ]");
            exit(0);
    ...
    public function childSignalHandler($signo, $pid = null, $status = null) {
        ...
        while ($pid > 0) {
            if ($pid && isset($this->currentJobs[$pid])) {
                ...
                if ($exitCode != 0) {
                    Logger::error(get_class($this) . ": $pid exited with status " . $exitCode);
                }
