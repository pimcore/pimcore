...
<form action="<?= $urlSave ?>" method="post">
    ...
    <div class="modal-body">
        <div class="row form-group">
            <label class="col-sm-12" for="inputMessage"><?= $this->translateAdmin('bundle_ecommerce.back-office.order.item-cancel.message') ?></label>
            <div class="col-sm-12"><textarea class="form-control" name="message" rows="8" id="inputMessage"></textarea></div>
        </div>
    </div>
    <div class="modal-footer">
        <button type="submit" class="btn btn-danger" name="confirmed" value="1"><?= $this->translateAdmin('bundle_ecommerce.back-office.order.item-cancel') ?></button>
    </div>
</form>