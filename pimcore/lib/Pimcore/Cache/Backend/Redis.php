...
class Redis extends \Zend_Cache_Backend implements \Zend_Cache_Backend_ExtendedInterface
{
    ...
    public function ___expire($id)
    {
        ...
        $this->_conn->zAdd(
             $this->_options['prefix'].':expiry', $expire, $id
        );
    }
        ...
        switch ($mode) {
            case \Zend_Cache::CLEANING_MODE_ALL:
                ...
                foreach ($toDelete as $key) {
                    if ($key === $this->_options['prefix'].':datastructure_version') {
                        continue;
                    }
                    $this->_conn->del($key);
                }
    ...
    public function getIdsMatchingTags($tags = array())
    {
        ...
        if (0 == count($tags)) {
            return $ret;
        } // don't do anything if no tags passed
        ...
        */
        ...
        foreach ($tags as $tag) {
            ...
            if (0 == $cardinality) {
                break;
            } // stop iteration if no keys left
        }
    ...
    public function getFillingPercentage()
    {
        ...
        return round(
                     ($info['used_memory']/$maxMem['maxmemory']*100), 0, PHP_ROUND_HALF_UP
                );
    }
    ...
    public function touch($id, $extraLifetime)
    {
        ...
        $this->_conn->zRem(
             $this->_options['prefix'].':expiry', $id
        );
        $this->_conn->zAdd(
             $this->_options['prefix'].':expiry', $data['expire'], $id
        );
        return true;
    }
    ...
    protected function set($id, $data, $lifetime, $tags)
    {
        ...
        // write expiry
        $this->_conn->zAdd(
             $this->_options['prefix'].':expiry', $h['expire'], $id
        );
        return true;
    }
