...
class Reports_AnalyticsController extends Pimcore_Controller_Action_Admin_Reports {
    ...
    public function siteoverviewAction () {
		...
		$startDate = date("Y-m-d",$this->_getParam("dateFrom"));
		$endDate = date("Y-m-d",$this->_getParam("dateTo"));
		...
		foreach($result as $row){
            ...
            $dailyDataGrouped["pageviews"][] = (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_PAGEVIEWS)->getValue();
            $dailyDataGrouped["visits"][] = (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_VISITS)->getValue();
            $dailyDataGrouped["uniquepageviews"][] = (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_UNIQUE_PAGEVIEWS)->getValue();
            $dailyDataGrouped["exits"][] = (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_EXITS)->getValue();
            $dailyDataGrouped["timeonsite"][] = (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_TIME_ON_SITE)->getValue();            
            $dailyDataGrouped["bounces"][] = (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_BOUNCES)->getValue(); 
            
            if((int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_VISITS)->getValue() > 0) {
                $dailyDataGrouped["pagespervisit"][] = (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_PAGEVIEWS)->getValue() / (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_VISITS)->getValue(); 
            }
            
            if((int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_ENTRANCES)->getValue() > 0) {
                $dailyDataGrouped["bouncerate"][] = (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_BOUNCES)->getValue() / (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_ENTRANCES)->getValue();
            }
            
            $dailyData[] = array(
                "pageviews" => (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_PAGEVIEWS)->getValue(),
                "uniquepageviews" => (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_UNIQUE_PAGEVIEWS)->getValue(),
                "exits" => (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_EXITS)->getValue(),
                "timeonsite" => (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_TIME_ON_SITE)->getValue(),
                "date" => $date->get(Zend_Date::DATE_MEDIUM),
                "visits" => (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_VISITS)->getValue(),
                "bounces" => (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_BOUNCES)->getValue(),
                "bouncerate" => ((int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_ENTRANCES)->getValue()) ? ((int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_BOUNCES)->getValue() / (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_ENTRANCES)->getValue()) : 0
            );
            
            $data["bounces"] += (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_BOUNCES)->getValue();
            $data["entrances"] += (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_ENTRANCES)->getValue();
            $data["exits"] += (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_EXITS)->getValue();
            $data["pageviews"] += (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_PAGEVIEWS)->getValue();
            $data["uniquepageviews"] += (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_UNIQUE_PAGEVIEWS)->getValue();
            $data["timeonsite"] += (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_TIME_ON_SITE)->getValue();
            $data["visits"] += (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_VISITS)->getValue();
            if((int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_VISITS)->getValue() > 0) {
                $data["pagespervisit"] += (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_PAGEVIEWS)->getValue() / (int) $row->getMetric(Zend_Gdata_Analytics_DataQuery::METRIC_VISITS)->getValue();
            }
        }
                
        $data["pagespervisit"] = $data["pagespervisit"] / count($dailyData);
        $data["timeonsite"] = $this->formatDuration($data["timeonsite"] / $data["visits"]);
        $data["exitPercentage"] = ($data["exits"]/$data["pageviews"]) * 100;
        $data["bouncerate"] = $data["bounces"] / $data["entrances"] * 100;
        
        $this->view->dailyData = $dailyData;
        $this->view->data = $data;
        $this->view->dailyDataGrouped = $dailyDataGrouped;
    }
    */
