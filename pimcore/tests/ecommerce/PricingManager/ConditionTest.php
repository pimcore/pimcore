...
class ConditionTest extends EcommerceTestCase
{
    ...
    public function testCartAmount() {
        ...
        $cartAmount = new CartAmount();
        $cartAmount->setLimit(100);
        $cartAmount->setMode(CartAmount::CALCULATION_MODE_PRODUCT_AND_CART);
        ...
        $cartAmount = new CartAmount();
        $cartAmount->setLimit(199);
        $cartAmount->setMode(CartAmount::CALCULATION_MODE_PRODUCT_AND_CART);

        $this->assertTrue($cartAmount->check($environment), "check not empty product");
