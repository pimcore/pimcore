...
pimcore.asset.helpers.gridConfigDialog = Class.create({
    ...
    getMetadataTreePanel: function () {
        if (!this.metadataTreePanel) {
            ...
            this.brickKeys = [];
            this.metadataTreePanel = this.getMetatdataTree("/admin/asset-helper/get-metadata-for-column-config");
        }

        return this.metadataTreePanel;
    },

    getMetatdataTree: function (url) {

        var metatdataTreeHelper = new pimcore.object.helpers.metadataTree(this.showFieldname);
        var tree = metatdataTreeHelper.getMetaTree(url, 0, 0);
    ...
    getOperatorTrees: function () {
        var operators = Object.keys(pimcore.asset.gridcolumn.operator);
        var operatorGroups = [];
        // var childs = [];
        for (var i = 0; i < operators.length; i++) {
            var operator = operators[i];
            if (!this.availableOperators || this.availableOperators.indexOf(operator) >= 0) {
                var nodeConfig = pimcore.asset.gridcolumn.operator[operator].prototype;
