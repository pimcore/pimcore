...
pimcore.asset.helpers.gridColumnConfig = {
        ...
        var configPanel = new Ext.Panel({
            ...
            buttons: [{
                ...
                handler: function () {
                    ...
                    pimcore.helpers.saveColumnConfig(this.element.data.id, this.element.data.id, this.getGridConfig(), this.searchType, this.saveColumnConfigButton,
                        this.columnConfigurationSavedHandler.bind(this), this.settings, "asset");
    ...
    deleteGridConfigConfirmed: function (btn) {
        if (btn == 'ok') {
            Ext.Ajax.request({
                url: "/admin/asset-helper/grid-delete-column-config",
                method: "DELETE",
                params: {
    ...
    buildColumnConfigMenu: function (onlyConfigs) {
        ...
        if (!onlyConfigs) {
            ...
            menu.add({
                ...
                handler: function () {
                    pimcore.helpers.markColumnConfigAsFavourite(0, this.element.data.id, this.settings.gridConfigId, this.searchType, true, "asset");
                }.bind(this)
            });
