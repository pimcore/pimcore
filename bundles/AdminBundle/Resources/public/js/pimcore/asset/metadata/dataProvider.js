...
pimcore.asset.metadata.dataProvider = Class.create({
    ...
    buildKeyFromItem: function (item) {
        let key = this.buildKey(item.name, item.language);
        return key;
    },
    ...
    unregisterChangeListener: function (key, targetId) {
        ...
        if (Object.keys(this.changeListeners[key]).length == 0) {
            // no UI elements anymore, delete the data as well
    ...
    getItemData: function (name, language) {
        var item = this.getItem(name, language);
        if (item) {
            return item.data;
        }
    },

    getItem: function (name, language) {
        let key = this.buildKey(name, language);
        let value = this.store[key];
        return value;
    },

    buildKey: function (name, language) {
        language = language || "";
            ...
            if (this.changeListeners[key]) {
                for (let targetId in this.changeListeners[key]) {
                    if (this.changeListeners[key].hasOwnProperty(targetId)) {
                        ...
                        callback(eventType, item.name, item.language, item.data, originator);
                    }
                }
            }
        } catch (e) {
