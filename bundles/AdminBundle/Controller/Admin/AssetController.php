...
class AssetController extends ElementControllerBase implements EventedControllerInterface
{
    ...
    public function deleteAction(Request $request)
    {
        if ($request->get('type') == 'childs') {
            ...
            $list->setCondition('path LIKE ?', [Db::get()->escapeLike($parentAsset->getRealFullPath()) . '/%']);
    ...
    protected function getTreeNodeConfig($element)
    {
        ...
        if ($asset->getType() == 'folder') {
            ...
            $children->setCondition('path LIKE ?', [Db::get()->escapeLike($asset->getRealFullPath() . '/%')]);
    ...
    public function getFolderContentPreviewAction(Request $request, EventDispatcherInterface $eventDispatcher)
    {
        ...
        $conditionFilters = [];
        $list = new Asset\Listing();
        $conditionFilters[] = 'path LIKE ' . ($folder->getRealFullPath() == '/' ? "'/%'" : $list->quote(Db::get()->escapeLike($folder->getRealFullPath() . '/%'))) ." AND type != 'folder'";
    ...
    public function copyInfoAction(Request $request)
    {
        ...
        if ($request->get('type') == 'recursive') {
            ...
            if ($asset->hasChildren()) {
                ...
                $list = new Asset\Listing();
                $list->setCondition('path LIKE ?', [Db::get()->escapeLike($asset->getRealFullPath() . '/%')]);
                $list->setOrderKey('LENGTH(path)', false);
                $list->setOrder('ASC');
    ...
    public function downloadAsZipJobsAction(Request $request)
    {
        ...
        if ($asset->isAllowed('view')) {
            ...
            $conditionFilters[] = 'path LIKE ' . $db->quote(Db::get()->escapeLike($parentPath . '/%')) .' AND type != ' . $db->quote('folder');
            if (!$this->getAdminUser()->isAdmin()) {
                $userIds = $this->getAdminUser()->getRoles();
                $userIds[] = $this->getAdminUser()->getId();
                $conditionFilters[] = ' (
    ...
    public function downloadAsZipAddFilesAction(Request $request)
    {
        ...
        if ($asset->isAllowed('view')) {
            ...
            if ($zipState === true) {
                ...
                $conditionFilters[] = "type != 'folder' AND path LIKE " . $db->quote(Db::get()->escapeLike($parentPath . '/%'));
                if (!$this->getAdminUser()->isAdmin()) {
                    $userIds = $this->getAdminUser()->getRoles();
                    $userIds[] = $this->getAdminUser()->getId();
                    $conditionFilters[] = ' (
