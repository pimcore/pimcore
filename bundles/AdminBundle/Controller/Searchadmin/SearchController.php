                ...
                foreach ($types as $type) {
                    if (($allParams['considerChildTags'] ?? 'false') === 'true') {
                        ...
                        if ($tag) {
                            ...
                            $conditionParts[] = 'id IN (SELECT cId FROM tags_assignment INNER JOIN tags ON tags.id = tags_assignment.tagid WHERE ctype = ' . $db->quote($type) . ' AND (id = ' .(int)$tagId. ' OR idPath LIKE ' . $db->quote(Helper::escapeLike($tagPath) . '%') . '))';
                        }
                    } else {
