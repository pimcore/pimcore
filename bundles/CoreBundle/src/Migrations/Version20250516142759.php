 ...
 */
final class Version20250516142759 extends AbstractMigration
{
    public function getDescription(): string
    {
        return 'Introducing 3 indices for optimizing maintenance on versions table';
    }
    ...
    public function down(Schema $schema): void
    {
        $table = $schema->getTable('versions');
        if ($table->hasIndex('ctype_public_id')) {
            $table->dropIndex('ctype_public_id');
        }

        if ($table->hasIndex('ctype_public_id_date')) {
            $table->dropIndex('ctype_public_id_date');
        }

        if ($table->hasIndex('ctype_public_id_cid')) {
            $table->dropIndex('ctype_public_id_cid');
        }
    }
}
