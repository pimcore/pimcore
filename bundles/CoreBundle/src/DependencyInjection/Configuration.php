...
final class Configuration implements ConfigurationInterface
{
                                        ...
                                        ->children()
                                            ...
                                            ->arrayNode('options')
                                                ->children()
                                                    ...
                                                    ->append($this->buildWorkflowNotificationSettingsNode('transition'))
                                ...
                                ->arrayNode('globalActions')
                                    ->prototype('array')
                                        ->children()
                                            ...
                                            ->end()
                                            ->append($this->buildWorkflowNotificationSettingsNode('global action'))
                                            ->enumNode('changePublishedState')
                                                ->values([ChangePublishedStateSubscriber::NO_CHANGE, ChangePublishedStateSubscriber::FORCE_UNPUBLISHED, ChangePublishedStateSubscriber::FORCE_PUBLISHED, ChangePublishedStateSubscriber::SAVE_VERSION])
                                                ->defaultValue(ChangePublishedStateSubscriber::NO_CHANGE)
                                                ->info('Change published state of element while the global action is applied (only available for documents and data objects).')
                                            ->end()
                                            ->enumNode('unsavedChangesBehaviour')
                                                ->values([Transition::UNSAVED_CHANGES_BEHAVIOUR_SAVE,
                                                          Transition::UNSAVED_CHANGES_BEHAVIOUR_WARN,
                                                          Transition::UNSAVED_CHANGES_BEHAVIOUR_IGNORE, ])
                                                ->defaultValue(Transition::UNSAVED_CHANGES_BEHAVIOUR_WARN)
                                                ->info('Behaviour when the global action gets applied but there are unsaved changes')
                                            ->end()
    ...
    /**
     * Builds the `notificationSettings` node. Transitions and global actions promise identical
     * notification behaviour, so they share one definition instead of two copies that can drift apart.
     *
     * @param string $actionType how the action is named in the descriptions, e.g. `transition`
     */
    private function buildWorkflowNotificationSettingsNode(string $actionType): ArrayNodeDefinition
    {
        /** @var ArrayNodeDefinition $node */
        $node = (new TreeBuilder('notificationSettings'))->getRootNode();
                ...
                ->children()
                    ...
                    ->arrayNode('notifyUsers')
                        ->prototype('scalar')
                            ->cannotBeEmpty()
                        ->end()
                        ->info(sprintf('Send an email notification to a list of users (user names) when the %s gets applied', $actionType))
                    ->end()
                    ->arrayNode('notifyRoles')
                        ->prototype('scalar')
                            ->cannotBeEmpty()
                        ->end()
                        ->info(sprintf('Send an email notification to a list of user roles (role names) when the %s gets applied', $actionType))
                    ->end()
