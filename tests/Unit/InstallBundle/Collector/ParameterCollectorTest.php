...
final class ParameterCollectorTest extends TestCase
{
    ...
    public function testInteractiveSecretWithDefaultValueUsesDefaultOnEnter(): void
    {
        $definition = $this->createSimpleDefinition(
            'test',
            true,
            [new ConfigParameter(
                ...
                'Secret Key',
                ParameterType::Secret,
                defaultValue: 'auto-generated-key-123',
            )],
        );
        ...
        // No env var, but defaultValue is set: pressing Enter should use it
        $this->assertSame(['MY_SECRET' => 'auto-generated-key-123'], $values);
    }

    public function testInteractiveSecretWithDefaultValueShowsAutoGeneratedHint(): void
    {
        $definition = $this->createSimpleDefinition(
            'test',
            true,
        ...
        $this->assertStringContainsString('auto-generated default', $rendered);
        // Should NOT say "already configured" (that's only for env vars)
        $this->assertStringNotContainsString('already configured', $rendered);
    }

    public function testInteractiveSecretWithDefaultValueCanBeOverridden(): void
    {
        $definition = $this->createSimpleDefinition(
            'test',
            true,
        ...
        // Simulate user typing a custom value instead of pressing Enter
        $io = $this->createInteractiveIo("custom-secret\n");
        $values = $this->collector->collect($definition, $io, true);

        $this->assertSame(['MY_SECRET' => 'custom-secret'], $values);
    }

    public function testNonInteractiveSecretUsesDefaultWhenNoEnvVar(): void
    {
        $definition = $this->createSimpleDefinition(
            'test',
            true,
            [new ConfigParameter(
                ...
                'Secret Key',
                ParameterType::Secret,
                defaultValue: 'auto-generated-key-456',
            )],
        );
        ...
        // Non-interactive: no env var, but defaultValue set → uses default
        $this->assertSame(['MY_SECRET' => 'auto-generated-key-456'], $values);
    }
